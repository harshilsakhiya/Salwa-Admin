...
const NewDashboardPage = () => {
  const { categoryId, serviceId, subserviceIndex } = useParams<{
    categoryId: string;
    serviceId: string;
    action: string;
    ...
    try {
      const response =
        await IndividualClinicService.GetAllForAdminIndividualClinicServiceRequests(
          {
            clinicSiteId: subService?.id ? parseInt(subService.id) : undefined,
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Set subservice from state
        if (state?.subService) {
          setSubService(state.subService);
        }
